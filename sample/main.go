...
func walkDir(obj string, fileChan chan string, wg *sync.WaitGroup) {
	...
	if err != nil {
		fmt.Println("walk dir %s error: %v \n", obj, err)
		return
	}
}
...
func walk(obj string, fileChan chan string) {
	...
	if f, err := os.Stat(obj); err != nil {
		fmt.Println("scan object %s error: %v \n", obj, err)
	} else {
		if f.IsDir() {
			...
			fmt.Println("invalid scan object: %s \n", obj)
		}
	}
	wg.Wait()
	fileChan <- EndSig
...
func generate_train_data(fileChan chan string, outputFile string) {
	...
	if err != nil {
		fmt.Println("open file %s failed: %v", outputFile, err)
		return
	}
	...
	for {
		...
		case obj := <-fileChan:
			...
			if content, err := ioutil.ReadFile(obj); err != nil {
				fmt.Println("read file %s error: %v", obj, err)
			} else {
				var param []string
				...
				if err != nil {
					fmt.Println("write file %s error: %v", outputFile, err)
				}
			}
		}
	}
