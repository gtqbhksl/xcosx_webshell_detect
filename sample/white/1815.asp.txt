...
Class UpFile_Cls
	...
	Public ErrCodes,ObjName,UploadFiles,UploadForms,Count,CountSize,ReName
	'-----------------------------------------------------------------------------------
	'初始化类
	'-----------------------------------------------------------------------------------
			...
			If IsObject(UploadObj.Form(FormName)) Then
				If Not UploadObj.Form(FormName).IsEmpty Then
					...
					If CheckFileExt(FileExt) = False then
						...
						EXIT SUB
					End If
					FileName = FormatName(FileExt,FileName)
					FileType = CheckFiletype(FileExt)
					'If IsBinary Then
					...
					If Filesize>0 Then
						...
						AddData FormName , _ 
								...
								-1 , _
								-1 , _
								FixFileName(UploadObj.Form(FormName).ShortFileName)
						Count = Count + 1
	...
	Private Sub AddData( Form_Name,File_Name,File_Path,File_Size,File_ContentType,File_Type,File_Data,File_Ext,File_Width,File_Height,File_OldName)
		Set FileInfo = New FileInfo_Cls
			...
			FileInfo.FileOldName = File_OldName
